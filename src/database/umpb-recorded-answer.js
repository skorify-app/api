export const get = async(conn, scoreId) => {
	return conn.query('SELECT * FROM umpb_recorded_answers WHERE score_id = ?', [scoreId]);
}

export const insert = async(conn, scoreId, answers) => {
	const prepare = await conn
	.prepare('INSERT INTO umpb_recorded_answers(score_id, question_id, answer) VALUES (?, ?, ?);');
